        ...
        <div className={`${styles.container} ${className ?? ""}`}>
            ...
            featureFlags?.ENABLE_WEB_CHAT && featureFlags?.ENABLE_UNGROUNDED_CHAT ?
                <ButtonGroup className={`${styles.buttonGroup}`} onClick={onClick} bsPrefix="ia">
                    <Button className={`${defaultValue == ChatMode.WorkOnly? styles.buttonleftactive : styles.buttonleft ?? ""}`} size="sm" value={0} bsPrefix='ia'>{"Grounded"}</Button>
            ...
            : // If neither WorkPlusWeb nor Ungrounded are enabled, show only WorkOnly button
                <ButtonGroup className={`${styles.buttonGroup}`} onClick={onClick} bsPrefix="ia">
                    <Button className={`${defaultValue == ChatMode.WorkOnly? styles.buttonmiddleactive : styles.buttonmiddle ?? ""}`} size="sm" value={0} bsPrefix='ia'>{"Grounded"}</Button>
                </ButtonGroup>
                }
        </div>
    );
